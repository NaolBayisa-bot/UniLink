...
export const PHOTO_ROOT_FOLDER = 'unilink/users';
...
export function userFolder(userId: string): string {
  return `${PHOTO_ROOT_FOLDER}/${userId}`;
}
...
export function createUploadSignature(
  ...
  const signature = cloudinary.v2.utils.api_sign_request(params, api_secret);
 ...
 */
export const PHOTO_BLUR_TRANSFORMATION = [{ effect: 'blur:2000' }];
...
export function buildPhotoUrl(
  ...
  return cloudinary.v2.utils.url(publicId, {
    ...
    cloud_name,
    api_secret,
    ...(opts.blurred ? { transformation: PHOTO_BLUR_TRANSFORMATION } : {}),
  });
}
