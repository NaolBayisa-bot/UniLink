...
import { upsertUserByTelegram, UpsertUserInput } from '../db/users';
 ...
 * steps above.
 */
export function createCompleteSignupHandler(
  deps: CompleteSignupDeps = {},
): RequestHandler {
  const doVerifyInitData = deps.verifyInitData ?? verifyInitData;
  const doVerifyGoogle = deps.verifyGoogleIdToken ?? verifyGoogleIdToken;
  const doUpsert = deps.upsertUserByTelegram ?? upsertUserByTelegram;
  const doSignToken = deps.signToken ?? signToken;

  return async (req, res) => {
    const { initDataRaw, googleIdToken } = (req.body ?? {}) as CompleteSignupBody;
 ...
 */
export function createAuthRouter(deps: CompleteSignupDeps = {}): Router {
  const router = Router();
